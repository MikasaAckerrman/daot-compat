 ...
 * airship stays at a fixed world point while the airship sails on. Its fuse explosion then
 * goes off where the spear was thrown, not where it landed, and the synced wire trails off.
 * We anchor the spear in sub-level local space on the first lodged tick and re-place it from
 * the sub-level's current pose every server tick after that.
 */
